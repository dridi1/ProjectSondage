...
# ----------------------------
# Sidebar: File Uploader + Project Info
# ----------------------------

with st.sidebar:
    st.header("Projet Théorie de sondage")
    st.write("**2ᵉ année • Année universitaire : 2024-2025**")
    st.write("École supérieure de la statistique et l’Analyse de l’information")
    st.title("**Dridi Slim**")
    ...
    if submit_strat:
        ...
        if aux:
            if pd.api.types.is_numeric_dtype(df[aux]):
                ...
                alloc = alloc.merge(
                    props.pivot(index=strat, columns=aux, values='Pct').reset_index(),
                    on=strat
                )

        sampled = pd.concat([
            df[df[strat] == g].sample(min(s, len(df[df[strat] == g])))
            for g, s in nh.items()
        ])
